...
public final class Usp implements SharedPreferences.Editor, SharedPreferences {
    ...
    private Usp(String spName) {
        if (TextUtils.isEmpty(spName)) {
            // 默认的sp名称: 包名_SP
            spName = App.instance.getApplicationContext().getPackageName() + "_SP";
        }
        mSharedPreferences = App.instance.getApplicationContext().getSharedPreferences(spName, Context.MODE_PRIVATE);
    }
    ...
    public static Usp get() {
        if (sInstance == null) {
            synchronized (Usp.class) {
                if (sInstance == null) {
                    sInstance = new Usp(NAME);
                }
            }
        }
